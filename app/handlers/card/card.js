...
async function updateLabels(request) {
    let cardDao = new CardDAO(request.app.db);
    let listDao = new ListDAO(request.app.db);
    let boardDao = new BoardDAO(request.app.db);
    ...
    return { message: await cardDao.updateLabels(request.payload.cardID, request.payload.colors) };
}
...
module.exports.updateLabels = updateLabels